...
{
    DistanceConstraint::DistanceConstraint(RigidBody3D* obj1, RigidBody3D* obj2, const glm::vec3 &globalOnA,
                                           const glm::vec3 &globalOnB)
            : m_pObj1(obj1), m_pObj2(obj2)
    {
    ...
    void DistanceConstraint::ApplyImpulse()
    {
        ...
        float constraintMass = (m_pObj1->GetInverseMass() + m_pObj2->GetInverseMass()) + glm::dot(abn, glm::cross(
                m_pObj1->GetInverseInertia() * glm::cross(r1, abn), r1) + glm::cross(
                m_pObj2->GetInverseInertia() * glm::cross(r2, abn), r2));

        float b = 0.0f;
        {
            float distanceOffset = glm::length(ab) - m_Distance;
            float baumgarteScalar = 0.1f;
            b = -(baumgarteScalar / PhysicsEngine::GetDeltaTime()) * distanceOffset;
        }
        ...
        m_pObj1->SetAngularVelocity(
                m_pObj1->GetAngularVelocity() + m_pObj1->GetInverseInertia() * glm::cross(r1, abn * jn));
        m_pObj2->SetAngularVelocity(
                m_pObj2->GetAngularVelocity() - m_pObj2->GetInverseInertia() * glm::cross(r2, abn * jn));
    }
