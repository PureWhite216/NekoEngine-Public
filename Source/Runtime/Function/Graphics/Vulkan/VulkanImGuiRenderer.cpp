...
namespace NekoEngine
{
    ...
    void VulkanImGuiRenderer::FrameRender(ImGui_ImplVulkanH_Window* wd)
    {
                ...
                for(int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
                {
                    ...
                    if((Texture*) pcmd->TextureId)
                    {
                        ...
                        else if(((Texture*) pcmd->TextureId)->GetType() == TextureType::DEPTH)
                        {
                            ...
                            texture->TransitionImage(VK_IMAGE_LAYOUT_DEPTH_STENCIL_READ_ONLY_OPTIMAL,
        ...
        m_Renderpass->BeginRenderpass(currentCommandBuffer, clearColour, m_Framebuffers[wd->FrameIndex],
                                      SubPassContents::INLINE, wd->Width, wd->Height);

        {
            // Record Imgui Draw Data and draw funcs into command buffer
            ImGui_ImplVulkan_RenderDrawData(ImGui::GetDrawData(), currentCommandBuffer->GetHandle(), VK_NULL_HANDLE,
                                            wd->FrameIndex);
        }
